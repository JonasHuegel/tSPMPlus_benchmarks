...
toc()
tic("sparsity screening")
seq2deep <- data.table::rbindlist(dbseq)
seq2deep <- as.data.frame(seq2deep)
seq2deep$patient_num.y <- NULL
colnames(seq2deep)[3] <- "patient_num"
...
tryCatch({
	...
	local_uniqpats <- c(unique(seq2deep$patient_num)) 

	seq2deep.agg.cut <- subset(seq2deep.agg,seq2deep.agg$distinct_patients > round(length(local_uniqpats)*sparsity))
	sequences <- c(as.character(unique(seq2deep.agg.cut$seq2deep)))
	rm(seq2deep.agg.cut)
	gc()
	# seq2deep <- setDF(seq2deep)
	...
	rm(g,p,y,dbseq,cl,seq2deep,pat.dat.sequence)
	}, 
    error = function(fr) {cat("ERROR :",conditionMessage(fr), "\n")})
toc()
toc()
